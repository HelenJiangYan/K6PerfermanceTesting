...
export default function () {
  ...
  console.log('  K6 Test: Create Project (With Config Layer)');
  console.log('='.repeat(60));
...
export function handleSummary(data) {
  return {
    ...
    // HTML 报告（适合分享）
    'results/create-project-report.html': htmlReport(data, {
      title: '单次项目创建测试报告',
      description: `环境: ${data.setup?.env?.domain || 'Unknown'}, 测试类型: 功能验证`,
    }),

    // JSON 数据（便于后续分析）
    'results/create-project-data.json': JSON.stringify(data, null, 2),
  };
}

function textSummary(data) {
  let summary = '\n' + '='.repeat(60) + '\n';
  summary += ' CREATE PROJECT TEST SUMMARY\n';
  summary += '='.repeat(60) + '\n\n';
